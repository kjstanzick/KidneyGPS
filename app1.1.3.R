             ...
             tabPanel("Region",
                      ...
                      shinyjs::hidden(
						...
						tabPanel("Documentation & Help",
						         ...
						         p("Selection of gentetic loci and therefore the genes within those loci, which are presented here, is based on a ",tags$abbr(title="genome-wide association studies","GWAS")," meta-analysis for eGFRcrea of ", tags$a(href="https://www.ukbiobank.ac.uk/", target="_blank", "UK Biobank data"), "and ", tags$a(href="https://ckdgen.imbi.uni-freiburg.de", target="_blank", "CKDGen consortium"), "data", tags$b("(n=1,201,909)."),"
						           Detailed information on the selection process can be found ",tags$a(href="https://www.nature.com/articles/s41467-021-24491-0", target="_blank", "here.")," A GWAS-metaanlysis of the same data restricted to European-ancestry (n=1,004,040) was used to identify independent
...
# GPS ---------------------------------------------------------------------
  ...
  $(thead).closest('thead').find('th').eq(17).css('background-color', '#ccffff');
  ...
  $(thead).closest('thead').find('th').eq(20).css('background-color', '#ffdb99');
  ...
  output$GPS <- renderDataTable({
    ...
    datatable(
      GPS_show[,c(4,1:3,9,10,24,23,11:19,22,20,21,5:8)], rownames=FALSE,
  ...
  output$GPS_10 <- renderDataTable({
    ...
    datatable(
      GPS_10_show[,c(4,1:3,9,10,24,23,11:19,22,20,21,5:8)], rownames=FALSE,
  ...
  output$GPS_50 <- renderDataTable({
    ...
    datatable(
      GPS_50_show[,c(4,1:3,9,10,24,23,11:19,22,20,21,5:8)], rownames=FALSE,
...
# region page -------------------------------------------------------------
  ...
  $(thead).closest('thead').find('th').eq(11).css('background-color', '#b3ffb3');
  $(thead).closest('thead').find('th').eq(16).css('background-color', '#ccffff');
  $(thead).closest('thead').find('th').eq(17).css('background-color', '#ccffff');
  $(thead).closest('thead').find('th').eq(18).css('background-color', '#ccffff');
  $(thead).closest('thead').find('th').eq(19).css('background-color', '#ffdb99');
  $(thead).closest('thead').find('th').eq(20).css('background-color', '#ffdb99');
  $(thead).closest('thead').find('th').eq(21).css('background-color', '#ffdb99');
  $(thead).closest('thead').find('th').eq(22).css('background-color', '#ffdb99');
  $(thead).closest('thead').find('th').eq(23).css('background-color', '#ffdb99');
  ...
  GPS_region_search <- reactive({
    if(valid_reg()&overlap_loci()){
      x = GPS_show[which(GPS$locus_id%in%ov_loci()[,1]),c(4,1:3,9,10,24,11:19,22,20,21,5:8)]
...
# snp page ----------------------------------------------------------------
  ...
  output$CADD <- DT::renderDataTable({
    if(valid()){
      if(any(detail_evidence_variant()=="CADD")){
        if(any(CADD$RSID%in%cred_vars())){
          datatable(CADD_show[which(CADD$RSID%in%cred_vars()),], rownames=FALSE,
                    options = list(
                      columnDefs = list(list(className = 'dt-left', targets = "_all"))
                      , scrollX=TRUE,
                      dom = 'lfrtBip',
...
# gene page ---------------------------------------------------------------
    ...
    if(valid_gene()){
      if(any(columns()%in%c(11:13))){
        if(any(toupper(CADD$GeneName)%in%genes_in_gps())){
          datatable(CADD_show[which(toupper(CADD$GeneName)%in%genes_in_gps()&CADD$PPA>=ppa()),], rownames=FALSE,
                    options = list(
                      columnDefs = list(list(className = 'dt-left', targets = "_all"))
                      , scrollX=TRUE,
                      dom = 'lfrtBip',
  ...
  cadd_gene <- reactive({
    if(valid_gene()){
      if(any(columns()%in%c(11:13))){
        if(any(toupper(CADD$GeneName)%in%genes_in_gps())){
          CADD[which(toupper(CADD$GeneName)%in%genes_in_gps()&CADD$PPA>=ppa()),]
        }else{0}
      }else{0}
    }else{0}
  })
  ...
  GPS_gene <- reactive({
    if(valid_gene()){
        ...
        x=GPS_show[which(toupper(GPS$Gene)%in%genes_in_gps()),c(1:10,24,20,21,22)]
       ...
       htmltools::withTags(table(
              ...
              thead(
                tr(
                  ...
                  lapply(c('Distance to locus lead variant','Chromosome','Position gene start','Position gene end'),th, rowspan=2)
                ),
                tr(
