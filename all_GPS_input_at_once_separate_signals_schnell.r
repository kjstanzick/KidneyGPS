...
for (i in 1:nrow(GPS_table)){
	...
	d=merge(x,a,by.x="RSID",by.y="rsid",all=FALSE)
...
cadd_15_neu=merge(cadd_15, cred_var_99, by.x="RSID",by.y="rsid")
	
#cadd_15_neu=cadd_15_neu[which(!is.na(cadd_15_neu$ConsScore)),]
...
#	if(!any(sqtl$gene[i]==real_genes$approved_symbol) & any(sqtl$gene[i]==real_genes$genes)){
	
#		sqtl$gene[i]=real_genes$approved_symbol[which(real_genes$genes==sqtl$gene[i])[1]]
#	}
#}
...
#sqtl=sqtl[-which(duplicated(sqtl[,c(1,3,17,18,31,32)]))]
...
gtex_kidney = fread("./lmm/all_loci/gtex_eqtl/all_cred_var_wakefield_with_gtex_right_gene_name_no_dup_kidney.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE,  data.table = FALSE)

gtex_without_kidney = fread("./lmm/all_loci/gtex_eqtl/all_cred_var_wakefield_with_gtex_right_gene_name_no_dup_without_kidney.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE,  data.table = FALSE)

sqtl_kidney = fread("./lmm/all_loci/gtex_sqtl/all_cred_var_wakefield_with_gtex_sqtl_right_gene_name_no_dup_kidney.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE,  data.table = FALSE)

sqtl_without_kidney = fread("./lmm/all_loci/gtex_sqtl/all_cred_var_wakefield_with_gtex_sqtl_right_gene_name_no_dup_without_kidney.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE,  data.table = FALSE)
...
for(i in 1:length(kidney_genes)){
	...
	disease = paste(unique(kidney_pheno$disease[which(kidney_pheno$gene==kidney_genes[i])]),collapse=" | ")
